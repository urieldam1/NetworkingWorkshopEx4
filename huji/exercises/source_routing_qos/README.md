...
## Step 4: Extend your solution

Change the program to handle both IPv4 forwarding and source routing at the same time.
Add an action (called ipv4_forward), which will forward packets that do not have the source routing header, as in the basic routing exercise.

Add QOS. First we have to change the ipv4_t header by splitting the TOS field into DiffServ and ECN fields. Remember to update the checksum block accordingly. Then, in the egress control block we must compare the protocol in IP header with IP protocols. Based on the traffic classes and priority, the diffserv flag will be set.
1. An ingress control block that checks the protocols and sets the ipv4.diffserv.
2. Assign different diffserv values to TCP versus UDP traffic

## Step 5: Run your extended solution

Use send_qos.py to send regular traffic. Verify it is received no using source routing.
Use send_qos.py with flag "--p=UDP" and "--p=TCP" to send different types of traffic.
Verify that your code has changed the QOS by capturing the output from receive.py, and filter on the tos values.
